...
## Introduction
In the last week we were first introduced to the use of the grammar of graphics using `florence`. Up until now, we have only recreated already existing charts (in our case from Du Bois' Paris exhibition). In addition, we have been mostly concerning ourselves with constructing individual, single charts. In this week, we will start building out from that basis and will move onto using a more modern, multi-dimensional dataset. Visualizations of such dataset, especially for academic use, can help to generate insights much more easily if we use multiple charts or visual views on the same dataset instead. These multiple views in a single visualization are often referred to as facets (see Chapter 12 VAD). This week we will be 'graduating' from making single charts to creating faceted visualizations.
...
Before we start to look at multiple aspects and variables, it is a good idea to do a quick sanity check to make sure the importing of our data goes OK. We will use the same pattern as in previous weeks, where we `export const data = [... data goes ...]` from a `data.js` file. We then `import { data } from './data.js'` in our `App.svelte` file.

I have already set this up for you in the below sandbox. In that same sandbox, we follow the same procedure and pattern as last week to create a scatterplot of price (y axis) and floor area (x axis). This is a good pair of variables for a sanity check: we know that there should be a positive relationship between these two variables so we would expect our graph to mirror that. The basic steps involve:

1. Importing our data.
2. Setting up two scales for the x and y axes – with appropriate domains for each variable.
3. Creating a `Graph` element with the appropriate properties
...
So far, we have used the provided data as-is. We import it and subsequently map it directly into a graphic representation. However, for many visualization – even something as simple as a histogram – we need to first transform and aggregate our data in different ways.
...
Once imported, we can convert our original data structure to a DataContainer with like so:

```js
const sales = new DataContainer(data)
```

This `sales` DataContainer has a number of useful features. We will start with implementing these two:

- It allows us to easily extract the [`domain` for specific columns](https://github.com/spatialnetworkslab/florence-datacontainer#domains-and-types). This can be used to automatically calculate the correct scales for the x and y axis.
- It allows us to [extract an entire column as an Array](https://github.com/spatialnetworkslab/florence-datacontainer#datacontainer_column). This is very useful because it allows us to use the `PointLayer` instead of a combination of the `{#each}` loop and the individual `Point` Marks.
...
The four coloured squares are placeholder for different facets of our HDB dataset. In this section, we will use 'fill in' each square with the following visualizations:
...
So to calculate the number of transactions per flat type, we can combine the `group_by` and `summarise` functions.
...
Once you have the data in the right structure, you can start to think about how you map the data to the specific properties (i.e. 'channels' or 'aesthetics') of each mark. In this case, let us put the categories of the `flat_type` on the y axis (`scaleY`) and the `total_count` on the x axis (`scaleX`).
...
::: div callout
Try to work through the remaining facets yourself. Hint: to create a histogram, use the [bin transformation](https://github.com/spatialnetworkslab/florence-datacontainer#datacontainer_bin), rather than the standard `group_by`.
:::
