<script>
  ...
  $: salesDistributionSelection = salesSelection
    .bin({ groupBy: 'resale_price', method: 'EqualInterval', numClasses: 20 })
    .summarise({ total_count: { resale_price: 'count' } })
  ...
  $: {
    if (selectedType === '') {
      salesSelection = sales
    } else {
      salesSelection = sales
        .filter(row => row.flat_type === selectedType)
    }
  }
</script>
    ...
    <Graphic width={825} height={825}>
      ...
      >
        <RectangleLayer 
          ...
          fill={color}
          key={salesPerType.column('flat_type')}
      ...
      <Section
        ...
        scaleY={scaleLinear().domain([0, salesDistributionSelection.domain('total_count')[1]])}
      >
        <RectangleLayer 
          x1={salesDistributionSelection.map('bins', bin => bin[0])}
          x2={salesDistributionSelection.map('bins', bin => bin[1])}
          y1={0}
          y2={salesDistributionSelection.column('total_count')}
          fill={color}
          transition={1000}
        />
        <XAxis tickCount={5} labelFontSize={8} title="Resale Price"/>
        <YAxis labelFontSize={8} title="Count" transition={1000}/>
      </Section>
       <Section
        x1={0}
        x2={400}
